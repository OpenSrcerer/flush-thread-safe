...
FlushFinder::FlushFinder(int flushes, bool verbose) :nFlushes{ flushes }, bVerbose{ verbose } // initializer list in constructor. I like them :)
{
	...
	mConsoleThread = std::thread([&]() {
		while (alive.load() || !mQueue.Empty()) {
			if (mQueue.Empty()) std::this_thread::sleep_for(std::chrono::milliseconds(1));
			else {
				std::cout << mQueue.Read();
			}
		}
		});
}
	...
	{ // extra scope to trigger timer destruction
		...
		while (inserter) {
			...
			if (bVerbose)	// bah, code clutter. But this might be interesting
				writeMessage("\rGenerating flush(es)... -> Checked pairs: " 
					+ std::to_string(CardGenerator::nGenerated) 
					+ " | Flushes found: " 
					+ std::to_string((flush ? ++count : count)));
		}
		writeMessage(timer.Stop());
	}
	// create a reader that spits out the strings
	StringReader reader(pData, nFlushes);
	// while the reader is valid
	while (reader)
		writeMessage("\t" +  std::string(reader()) + "\n"); // read
}

void FlushFinder::writeMessage(std::string const& msg) {
	mQueue.Add(msg);
}
